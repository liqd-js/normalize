...
const NORMALIZERS = [ '_type', '_any', '_requires', '_passes', '_each', '_convert', '_expand', '_default', '_required', '_strict' ];
...
function normalize_value( parent, property, value, root, path, errors, schema, options )
{
	for( let normalizer in schema )
	{
		...
		if( '_type' === normalizer )
		{
			...
			{
				errors[ path_str( path )] = 'invalid_type'; continue;
			}
		}
		else if( '_any' === normalizer )
		{
			...
			{
				errors[ path_str( path )] = 'invalid_value'; continue;
			}
		}
		else if( '_requires' === normalizer )
		{
			...
			{
				errors[ path_str( path )] = 'missing_requirement'; continue;
			}
		}
		...
		else if( '_each' === normalizer )
		{
			...
			{
				normalize_value( value, i, value[i], root, [ ...path, i ], errors, schema._each, options );
			}
		}
...
function normalize_object( obj, root, path, errors, schema, options )
{
	for( let property in obj )
	{
		...
		normalize_value( obj, property, obj[property], root, [ ...path, property, ], errors, schema[property], options );
	...
	{
		if( schema[property]._default && !obj.hasOwnProperty( property ))
		{
